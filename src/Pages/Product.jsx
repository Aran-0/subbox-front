...
import { loadProductByTitle } from "../data/products";
import { saveViewedProduct } from "../components/common/functions/recommendationUtils";
import {
  addItemToSubscription,
  removeItemFromSubscription,
  isProductInSubscription,
} from "../components/common/functions/subscriptionUtils";
...
const Product = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      const product = await loadProductByTitle(title);
      ...
      if (product) {
        ...
        setIsInSubscription(isProductInSubscription(product.id));
      }
    };
  ...
  const availableColors = selectedProduct?.colors || ["#111827", "#F97316", "#10B981", "#3B82F6"];
  ...
  const handleSubscriptionToggle = () => {
    if (!selectedProduct) return;
    if (isInSubscription) {
      removeItemFromSubscription(selectedProduct.id);
    } else {
      addItemToSubscription(selectedProduct);
    }
    setIsInSubscription(!isInSubscription);
  };
  ...
  if (loading) {
    return <div className="text-center py-20">Загрузка товара...</div>;
  }
          ...
          <div className="mx-auto  flex flex-col gap-10">
            ...
            <div className="flex flex-col md:flex-row  gap-16">
              ...
              <div className="flex gap-5 flex-col">
                ...
                <div className="font-inter text-xl">{i18n.t("productPage.colors")}: </div>
                <div className="flex items-center gap-4">
                  <div className="flex gap-3 items-center">
                    {availableColors.map((color) => (
                      <button
                        ...
                        aria-label={color}
                        title={color}
                        className={`w-8 h-8 rounded-full border-2 focus:outline-none transition-shadow duration-150 ${
                          selectedColor === color
                  ...
                  <div className="text-sm text-gray-500">
                    {selectedColor ? "" : i18n.t("redButtons.exploreByCategory")}
                  </div>
                </div>
