...
import AssistantDialog from "../common/components/Assistant";
...
const Profile = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const id = currentUser.uid || currentUser.email || "anonymous";
      const key = `sloganShownFor:${id}`;
      if (!localStorage.getItem(key)) {
        setSloganOpen(true);
        // mark shown for this user so it doesn't repeat
        try {
          localStorage.setItem(key, "1");
        } catch (e) {
          // ignore
        }
        // auto close after a short animation duration
        const t = setTimeout(() => setSloganOpen(false), 2400);
    ...
    <div className="flex justify-between md:justify-around items-center md:gap-4 md:ml-4 ">
        ...
        >
          <span
            className={`absolute ml-5 mb-5 px-1 rounded-full text-white text-sm ${
              wishlistItems.length == 0 ? "hidden " : "bg-pink-500"
            }`}
          >
            {wishlistItems.length}
          </span>
          <svg
            className="hover:text-pink-600 transition-colors duration-200 rounded-full w-6 md:w-8 md:h-8"
            viewBox="0 0 32 32"
            fill="none"
      ...
      <Link to="/cart">
        ...
        >
          <span
            className={`absolute ml-5 mb-5 px-1 rounded-full text-white text-sm ${
              cartItems.length == 0 ? "hidden " : "bg-indigo-600"
            }`}
          >
            {cartItems.length}
          </span>
          ...
          <svg
            className="hover:text-indigo-600 transition-colors duration-200 rounded-full w-6 md:w-8 md:h-8"
            viewBox="0 0 32 32"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
      ...
      >
        ...
        {currentUser && currentUser.email === "admin@subbox.com" && (
          <Link to="/admin">
            <MenuItem sx={{ display: "flex", gap: "16px" }} onClick={handleClose}>
              <svg
                width="24"
        ...
        <MenuItem sx={{ display: "flex", gap: "16px" }} onClick={() => { setAssistantOpen(true); handleClose(); }}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2Z" stroke="#FAFAFA" strokeWidth="1.5"/>
            <path d="M8 14H16" stroke="#FAFAFA" strokeWidth="1.5" strokeLinecap="round"/>
            <path d="M8 10H9" stroke="#FAFAFA" strokeWidth="1.5" strokeLinecap="round"/>
            <path d="M11 10H13" stroke="#FAFAFA" strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          AI Ассистент
        </MenuItem>
      ...
      {/* Readme component removed from header to hide App Readme */}
      <AnimatePresence>
        {sloganOpen && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <motion.div
              ...
              className="absolute inset-0 bg-gradient-to-br from-indigo-800 via-indigo-600 to-pink-500/80"
            ...
            <motion.h1
              ...
              className="relative text-white text-3xl md:text-5xl lg:text-6xl font-extrabold text-center px-6"
