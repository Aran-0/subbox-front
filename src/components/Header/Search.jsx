...
import { saveSearchHistory, saveViewedCategory } from "../common/functions/recommendationUtils";
...
const SearchAppBar = () => {
  ...
  const handleSearchSubmit = () => {
    ...
    const matchedItem = ITEMS.find(
      (item) => item.title?.toLowerCase() === cleaned.toLowerCase()
    );
    if (matchedItem) {
      saveViewedCategory(matchedItem.type);
    }

    navigate(`/allProducts/${encodeURIComponent(cleaned)}`);
  };
  ...
  return (
    <Search className="flex items-center justify-center w-48 min-[425px]:w-64 sm:max-[1200px]:w-96 min-[1200px]:w-60 min-[1450px]:w-96 ">
      ...
      />
      {searchText && (
        <IconButton aria-label="search" color="inherit" onClick={handleSearchSubmit}>
