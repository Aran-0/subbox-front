...
const FlashSaleItem = ({ item }) => {
  ...
  useEffect(() => {
    ...
    setIsInSubscription(isProductInSubscription(item.id, activeSubscription));
  }, [item]);

  const { handleAddToCart, isInCart } = useAddToCart(item);

  const handleSubscriptionToggle = () => {
    const activeSubscription = getActiveSubscription();
    if (isInSubscription) {
      removeItemFromSubscription(item.id, activeSubscription?.id);
      setIsInSubscription(false);
    } else {
      addItemToSubscription(item, activeSubscription?.id);
      setIsInSubscription(true);
    }
  };
  ...
  return (
    <div className="relative mx-2 ">
      ...
      >
        {isHovered && (
          <button
            ...
            className={`z-10 absolute bottom-0 left-0 right-0 bg-gradient-to-r from-indigo-600 to-indigo-500 text-white py-2 px-4 duration-300 hover:shadow-md focus:outline-none font-semibold ${
              isInCart && "bg-gradient-to-r from-green-500 to-green-600"
            }`}
          >
        ...
        {item.discount && (
          <div className="absolute top-0 left-0 bg-gradient-to-br from-amber-500 to-orange-500 text-white py-1 px-3 m-2 rounded font-semibold">
            -{item.discount}%
          </div>
        )}

        {item.state && (
          <div className="absolute top-0 left-0 bg-gradient-to-br from-green-500 to-emerald-600 text-white py-1 px-3 m-2 rounded font-semibold">
            {i18n.t("new")}
          </div>
        )}
      ...
      <div className="flex md:items-start items-center flex-col ">
        ...
        <p className="text-indigo-600 text-sm font-semibold line-clamp-2">
          ${item.price}
          {item.discount && (
        ...
        <Button
          variant={isInSubscription ? "contained" : "outlined"}
          size="small"
          onClick={handleSubscriptionToggle}
          sx={{ mt: 3, textTransform: "none" }}
        >
          {isInSubscription ? "In Subscription" : "Add to Subscription"}
        </Button>
        <span>
